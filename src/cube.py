...
class cube(object):
    ...
    def __init__(self):
        ...
        # No.0 stickers removed (fixed at location 111 and direction up now)
        self.STICKER_NUM = 7
        ...
        # Initial positions, also solved positions
    ...
    def turn(self, action):
        for i in range(7):
            # Change the position
            if self.state[i, 0] in self._action_map_position[action]:
                self.state[i, 0] = self._action_map_position[action][self.state[i, 0]]
    ...
    def peek_turn(self, action):
        ...
        for i in range(7):
            # Change the position
            if temp[i, 0] in self._action_map_position[action]:
                temp[i, 0] = self._action_map_position[action][temp[i, 0]]
